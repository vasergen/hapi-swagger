...
builder.getSwaggerJSON = async (settings, request) => {
    // -- changes
    const username = request.state && request.state.data && request.state.data.username;
    const filterEndpoints = username ? [username, 'public'] : ['public'];
    // -- changes end
    ...
    // -- changes
    routes = Filter.byTags([...filterEndpoints], routes);
